...
public class UpdateRecordServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		String intrest[]=request.getParameterValues("interest");
		//validate field
		...
		if("empty record".equals(errResponse))
			...
			StringBuffer intrest1=new StringBuffer();
			for(String hby:intrest)
				intrest1.append(hby+" ");
		try {
			Connection con=ConnectionDb.getConn();
			String command="update registration_details set fname=?,lname=?,gender=?,country=?,intrest=?,password=? where register_id=?";
